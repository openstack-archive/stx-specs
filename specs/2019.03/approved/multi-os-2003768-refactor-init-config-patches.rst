...
Current solutions just used for refactor patches and use config package or
puppet to package or change init/config files instead of existed patch.
No security impact.
...
patch and try the best to decrease the number of patch. We can also get benefit
when we consider multi-OS support on StarlingX.
For new joined developers, when we do some changes that refer to configuration
...
We have split the work to some tasks by SRPM package and plan to get it done
package by package.
...
4) Rebuilt and do deployment and related test to see if the change can be work
...
file is expected in specific foler.
For configuration file change scenario, we need to do additional patching test
and to see that if the configuration file is expected after patching.
