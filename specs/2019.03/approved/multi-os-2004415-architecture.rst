...
deb files. In order to give full support for another operating systems it is
necessary to refactor the current build model and generate a separation of
...
service and their packaging metadata (spec-files, deb runles), adding
appropriate infrastructure tooling to these components, such as autotools.
Autotools provides a mechanism to generate OS specific makefiles, setup.py
based on templates and ensures the correct buildtime dependencies are in place.
The "Source Reorg" specification will detail the proposed directory layout and
tools and build targets. Initially, the StarlingX flock could be built manually
and installed based on this new layout.
